...
class CustomsCalculator:
    ...
    def __init__(self, exchange_rates=None):
        ...
        # Таможенная пошлина для авто старше 5 лет (физические лица) в евро за 1 см³ (НОВОЕ ФОТО)
        self.duty_rates_over_5_years = [
            (0, 1000, 3.0),       # до 1000 см³: 3 € за см³
            (1000, 1500, 3.2),    # 1000-1500 см³: 3,2 € за см³
            (1500, 1800, 3.5),    # 1500-1800 см³: 3,5 € за см³
            (1800, 2300, 4.8),    # 1800-2300 см³: 4,8 € за см³
            (2300, 3000, 5.0),    # 2300-3000 см³: 5 € за см³
            (3000, float('inf'), 5.7)  # более 3000 см³: 5,7 € за см³
        ]
    ...
    def get_duty_for_age(self, engine_volume_cm3, age_days):
        ...
        if age_days < 3 * 365:  # Менее 3 лет (не включая день, когда исполняется 3 года)
            ...
            # Используем ставки для старше 5 лет (НОВОЕ ФОТО)
        ...
        if age_days <= 5 * 365:
            ...
            return engine_volume_cm3 * 5.7  # Для старше 5 лет
    ...
    def calculate_customs(self, purchase_price, currency, manufacture_date_str, engine_volume, hp, importer_type):
        ...
        if age_days < 3 * 365:  # Менее 3 лет (не включая день, когда исполняется 3 года)
            ...
            # Для автомобилей старше 5 лет - фиксированная пошлина по ДРУГИМ ставкам (НОВОЕ ФОТО)
            duty_euro = self.get_duty_for_age(engine_volume_cm3, age_days)
            customs_duty = duty_euro * self.exchange_rates['EUR']
...
async def handle_start_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Рассчитать таможню":
        ...
        info_text = (
            ...
            "• Старше 5 лет (5 лет и 1 день и более): фиксированная пошлина в евро по другим ставкам + утилизационный сбор\n\n"
            ...
            "• Для старше 5 лет (актуальные ставки):\n"
            "  - до 1000 см³ — 3 € за см³\n"
            "  - 1000–1500 см³ — 3,2 € за см³\n"
            "  - 1500–1800 см³ — 3,5 € за см³\n"
            "  - 1800–2300 см³ — 4,8 € за см³\n"
            "  - 2300–3000 см³ — 5 € за см³\n"
            "  - более 3000 см³ — 5,7 € за см³\n\n"
